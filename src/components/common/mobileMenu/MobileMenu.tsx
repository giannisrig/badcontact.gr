...
import { useMenuAnimation } from '@/components/common/mobileMenu/MobileMenu.hooks'

export default function MobileMenu() {
  const pathname = usePathname()
  const dispatch = useAppDispatch()
  const menuScopedContainer = useMenuAnimation()
  ...
  return (
    <header ref={menuScopedContainer} className={`fixed right-0 top-0 z-30 h-screen w-full text-white`}>
      <div className='flex h-full w-full flex-col'>
        <div className='flex items-center justify-between px-30px py-10px pb-5px'>
                  ...
                  <div className='flex w-full flex-col gap-25px'>
                    <div className='flex max-w-full flex-col gap-20px'>
                      <div className='flex max-w-full flex-col gap-5px'>
                        ...
                        <p className='font-primary text-sm italic'>
                          Disclaimer: This a demo NextJS project for education and showcase.
                        </p>
                      </div>
                      <p className='text-gray'>
                        A simple weather application that fetches weather data from an API and displays it in a visually
                        appealing way. Users can search for weather by location, view current weather conditions, and
